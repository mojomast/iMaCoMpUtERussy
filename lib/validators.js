...
export function validatePixelCoordinates(x, y, operationType = 'video', timeoutMs = 2000) {
  ...
  if (x < 0 || x > VALIDATION_CONFIG.MAX_PIXEL_COORDINATE ||
    ...
    throw new MCPError(MCP_ERROR_CODES.VIDEO_OUT_OF_BOUNDS, `Coordinates must be between 0 and ${VALIDATION_CONFIG.MAX_PIXEL_COORDINATE}`,
                      { field: 'coordinates', x, y, min: 0, max: VALIDATION_CONFIG.MAX_PIXEL_COORDINATE, display: `(${x}, ${y})` }, 422, true, 3, 1000, operationType, timeoutMs);
  }
}
...
export function createValidationErrorMapper(operationType = 'default', timeoutMs = 3000) {
  return {
    ...
    pixelCoordinates: (x, y) => new MCPError(MCP_ERROR_CODES.VIDEO_OUT_OF_BOUNDS, 'Invalid pixel coordinates',
                                           { x, y, expected: `0-${VALIDATION_CONFIG.MAX_PIXEL_COORDINATE}` }, 422, true, 3, 1000, operationType, timeoutMs),
