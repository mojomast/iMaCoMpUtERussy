...
export class MCPClient {
  ...
  constructor(baseURL = 'http://localhost:8001', options = {}) {
    ...
    this.cache = this.config.enableCache ? new ResponseCache(this.config.maxCacheSize, this.config.cacheTTLMs) : null;
  ...
  async interactiveDebug(assembly, breakpointLocations = []) {
    ...
    for (const addr of breakpointLocations) {
      try {
        ...
        console.warn(`Failed to set breakpoint at 0x${addr.toString(16)}:`, error.message);
      }
    }
  ...
  updateConfig(newConfig) {
    ...
    // Reinitialize cache if TTL changed
    if (newConfig.cacheTTLMs) {
      this.cache = this.config.enableCache ?
        new ResponseCache(this.config.maxCacheSize, this.config.cacheTTLMs) :
        null;
    }
  }
