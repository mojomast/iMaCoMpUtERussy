...
start:
    ...
    HLT
      ...
      // Note: This is a simplified check - in full implementation would capture all outputs
      expect(memory.readByte(0xF1)).toBe(0x0D); // Last character should be carriage return
...
fib_loop:
    ...
    HLT           ; Stop
      ...
      // Verify some intermediate values were computed correctly
      // Note: Exact memory locations depend on program, but we can check final results
...
start:
    LDA #$01      ; Green color
    VST #$00      ; Pixel at offset 0 (top-left)
    
    LDA #$02      ; Yellow color
    VST #$20      ; Pixel at offset 32 (next row)
    
    LDA #$03      ; Red color
    VST #$40      ; Pixel at offset 64
    
    VUP           ; Update display
    
    ; Simple pattern: fill first row with green
    LDX #$00
fill_row:
    LDA #$01      ; Green
    VST #X        ; Use X as offset (need indexed VST or loop)
    INX
    CPX #$20      ; 32 pixels per row
    BNE fill_row
    
    VUP           ; Update display again
    
    HLT
      `;
      
      // Note: The above source has limitations due to VST being immediate only
      // Create a simpler version that works with current instruction set
...
echo_loop:
    ...
    HLT
      ...
      while (cpu.running && steps < maxSteps && inputIndex < inputSequence.length) {
        ...
        if (currentPC === 0x0600 + 0 /* LDA $F2 offset */) {
        ...
        if (output === inputSequence[inputIndex] && output >= 0x20 && output <= 0x7E) {
          echoCount++;
...
.org 0x0600
    ...
    HLT
      ...
      // Verify terminal output "Status"
      // Note: Exact sequence verification is complex due to sequential writes
